export default class NotesView {
    constructor(root, { onNoteSelect, onNoteAdd, onPaintAdd, onNoteEdit, onNoteDelete } = {}) {
        this.root = root;
        this.onNoteSelect = onNoteSelect;
        this.onNoteAdd = onNoteAdd;
        this.onPaintAdd = onPaintAdd;
        this.onNoteEdit = onNoteEdit;
        this.onNoteDelete = onNoteDelete;
        this.root.innerHTML = `
                    <div class="header">
                        <div class="triangle">▽</div>
                        <div class="name">NoteQuill</div>
                        
                    </div>
                    <div class="notes__sidebar">
                        <button class="notes__add" type="button">Add Note</button>
                        <div class="notes__list"></div>
                    </div>
                
                    <div class="notes__preview">
                        <div class="choices">
                            <input placeholder="Search" type="text" id="searchInput">
                            <button id="save">Save</button>
                            <div id="btnDownload">Export</div>
                            <label id="fileInputLabel" for="fileInput" accept=".txt">Import</label>
                            <input type="file" id="fileInput" style="display: none;" name="import" placeholder="import"> 
                    
    
    
                            <button id="copy" class="copy">
                                <span>
                                    <svg xml:space="preserve" style="enable-background:new 0 0 512 512" viewBox="0 0 6.35 6.35" y="0" x="0" height="20" width="20" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg" class="clipboard">
                                        <g>
                                            <path fill="currentColor" d="M2.43.265c-.3 0-.548.236-.573.53h-.328a.74.74 0 0 0-.735.734v3.822a.74.74 0 0 0 .735.734H4.82a.74.74 0 0 0 .735-.734V1.529a.74.74 0 0 0-.735-.735h-.328a.58.58 0 0 0-.573-.53zm0 .529h1.49c.032 0 .049.017.049.049v.431c0 .032-.017.049-.049.049H2.43c-.032 0-.05-.017-.05-.049V.843c0-.032.018-.05.05-.05zm-.901.53h.328c.026.292.274.528.573.528h1.49a.58.58 0 0 0 .573-.529h.328a.2.2 0 0 1 .206.206v3.822a.2.2 0 0 1-.206.205H1.53a.2.2 0 0 1-.206-.205V1.529a.2.2 0 0 1 .206-.206z"></path>
                                        </g>
                                    </svg>
                                    <svg xml:space="preserve" style="enable-background:new 0 0 512 512" viewBox="0 0 24 24" y="0" x="0" height="18" width="18" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" xmlns="http://www.w3.org/2000/svg" class="checkmark">
                                        <g>
                                            <path data-original="#000000" fill="currentColor" d="M9.707 19.121a.997.997 0 0 1-1.414 0l-5.646-5.647a1.5 1.5 0 0 1 0-2.121l.707-.707a1.5 1.5 0 0 1 2.121 0L9 14.171l9.525-9.525a1.5 1.5 0 0 1 2.121 0l.707.707a1.5 1.5 0 0 1 0 2.121z"></path>
                                        </g>
                                    </svg>
                                </span>
                            </button>
                            
    
    
    
                        </div>
                        <input class="notes__title" type="text" placeholder="New Note...">
                        <textarea id="textarea" onclick="focus()" class="notes__body">Type here</textarea>
                        <p><span id="counter">0</span></p>
                    </div>
                    <nav class="toggleMode">
                        <div class="type"></div>
                        <div class="write"></div>
                        <p class="ttext1">Notes<p>
                        <p class="ttext2">Mind Map<p>
                        <div class="animate"></div>
                    </nav>
                `;
        

        const btnAddNote = this.root.querySelector(".notes__add");
        const inpTitle = this.root.querySelector(".notes__title");
        const inpBody = this.root.querySelector(".notes__body");

        btnAddNote.addEventListener("click", () => {
            this.onNoteAdd();
        });

        [inpTitle, inpBody].forEach(inputField => {
            inputField.addEventListener("blur", () => {
                const updatedTitle = inpTitle.value.trim();
                const updatedBody = inpBody.value.trim();

                this.onNoteEdit(updatedTitle, updatedBody);
                btnSave.style.borderColor = '#41de6669'
            });
        });
        const btnSave = document.getElementById('save')
        btnSave.addEventListener('click', () => {
            const updatedTitle = inpTitle.value.trim();
            const updatedBody = inpBody.value.trim();
            this.onNoteEdit(updatedTitle, updatedBody);
            btnSave.style.borderColor = '#41de6669'
        })
        const title = document.querySelector('.notes__title')
        title.addEventListener('input', () => {
            btnSave.style.borderColor = '#41de66'
        })
        const textarea = document.getElementById('textarea')
        textarea.addEventListener('input', () => {
            btnSave.style.borderColor = '#41de66'
        })
        

        this.updateNotePreviewVisibility(false);

        
        const writeDiv1 = document.querySelector('.write')
        const write1 = document.querySelector('.ttext2')
        writeDiv1.addEventListener("click", () => {
            const PaintApp = document.querySelector('.paint')
            const notesApp = document.querySelector('.notes')
            const notesPreview = document.querySelector('.notes__preview')
            const animate = document.querySelector(".animate")
            animate.style.left = '50%'
            notesApp.style.visibility = 'hidden'
            notesPreview.style.visibility = 'hidden'
            PaintApp.style.visibility = 'visible'
        })
        write1.addEventListener("click", () => {
            const PaintApp = document.querySelector('.paint')
            const notesApp = document.querySelector('.notes')
            const notesPreview = document.querySelector('.notes__preview')
            const animate = document.querySelector(".animate")
            animate.style.left = '50%'
            notesApp.style.visibility = 'hidden'
            notesPreview.style.visibility = 'hidden'
            PaintApp.style.visibility = 'visible'
        })


        const typeDiv1 = document.querySelector('.type')
        const type1 = document.querySelector('.ttext1')

        typeDiv1.addEventListener("click", () => {
            const notesApp = document.querySelector('.notes')
            const PaintApp = document.querySelector('.paint')
            const notesPreview = document.querySelector('.notes__preview')
            const animate = document.querySelector(".animate")
            animate.style.left = '50%'
            notesApp.style.visibility = 'visible'
            notesPreview.style.visibility = 'visible'
            PaintApp.style.visibility = 'hidden'

        })
        function criarArquivoTXT() {
            const conteudo = document.getElementById('textarea').value;
            const title = document.querySelector('.notes__title').value
            const blob = new Blob([conteudo], { type: 'text/plain' });
        
            const linkDownload = document.createElement('a');
            linkDownload.download = 'arquivo.txt';
            linkDownload.href = window.URL.createObjectURL(blob);
            linkDownload.dataset.downloadurl = ['text/plain', linkDownload.download, linkDownload.href].join(':');
            linkDownload.setAttribute('download', title);
            
            linkDownload.click();
        }

        
        const btnDownload = document.getElementById('btnDownload');

        document.addEventListener("DOMContentLoaded", function() {
            if (btnDownload) {
                btnDownload.addEventListener('click', function() {
                    criarArquivoTXT();
                });
            } else {
                console.error("O botão 'btnDownload' não foi encontrado.");
            }
        });


        function importarArquivo(event) {
            const arquivo = event.target.files[0];
            const leitor = new FileReader();
            const fileName = fileInput.files[0].name
            const fileNameWithoutExtension = fileName.replace('.txt', '')
        
            leitor.onload = function(e) {
                const conteudo = e.target.result;
                document.getElementById('textarea').value = conteudo;
            };
        
            leitor.readAsText(arquivo);
        
            const btnSave = document.getElementById('save')
            btnSave.style.borderColor = '#41de66'
        
            document.querySelector('.notes__title').value = fileNameWithoutExtension
        }
        
        document.getElementById('fileInput').addEventListener('change', importarArquivo);
        


        type1.addEventListener("click", () => {
            const notesApp = document.querySelector('.notes')
            const PaintApp = document.querySelector('.paint')
            const notesPreview = document.querySelector('.notes__preview')
            const animate = document.querySelector(".animate")
            animate.style.left = '-50%'
            notesApp.style.visibility = 'visible'
            notesPreview.style.visibility = 'visible'
            PaintApp.style.visibility = 'hidden'
        })
        
        window.addEventListener("DOMContentLoaded", (event) => {
            const typeDiv2 = document.querySelector('.type2')
            const type2 = document.querySelector('.ttext12')
            if (typeDiv2) {
                typeDiv2.addEventListener("click", () => {
                    const notesApp = document.querySelector('.notes')
                    const PaintApp = document.querySelector('.paint')
                    const notesPreview = document.querySelector('.notes__preview')
                    const animate = document.querySelector(".animate")
                    animate.style.left = '0%'
                    notesApp.style.visibility = 'visible'
                    notesPreview.style.visibility = 'visible'
                    PaintApp.style.visibility = 'hidden'

                })
            }

            if(type2) {
                type2.addEventListener("click", () => {
                    const notesApp = document.querySelector('.notes')
                    const PaintApp = document.querySelector('.paint')
                    const notesPreview = document.querySelector('.notes__preview')
                    const animate = document.querySelector(".animate")
                    animate.style.left = '0'
                    notesApp.style.visibility = 'visible'
                    notesPreview.style.visibility = 'visible'
                    PaintApp.style.visibility = 'hidden'
                })
            }




            const writeDiv = document.querySelector('.write2')
            const write = document.querySelector('.ttext22')
            writeDiv.addEventListener("click", () => {
                const PaintApp = document.querySelector('.paint')
                const notesApp = document.querySelector('.notes')
                const notesPreview = document.querySelector('.notes__preview')
                const animate = document.querySelector(".animate2")
                animate.style.left = '50%'
                notesApp.style.visibility = 'hidden'
                notesPreview.style.visibility = 'hidden'
                PaintApp.style.visibility = 'visible'
            })
            write.addEventListener("click", () => {
                const PaintApp = document.querySelector('.paint')
                const notesApp = document.querySelector('.notes')
                const notesPreview = document.querySelector('.notes__preview')
                const animate = document.querySelector(".animate2")
                animate.style.left = '50%'
                notesApp.style.visibility = 'hidden'
                notesPreview.style.visibility = 'hidden'
                PaintApp.style.visibility = 'visible'
            })
        })


       
        
    }


    _createListItemHTML(id, title, body, updated) {
        const MAX_BODY_LENGTH = 60;

        return `
            <div class="notes__list-item" data-note-id="${id}">
                <div class="notes__small-title">${title}</div>
                <div class="notes__small-body">
                    ${body.substring(0, MAX_BODY_LENGTH)}
                    ${body.length > MAX_BODY_LENGTH ? "..." : ""}
                </div>
                <div class="notes__small-updated">
                    ${updated.toLocaleString(undefined, { dateStyle: "full", timeStyle: "short" })}
                </div>
            </div>
        `;
    }

    updateNoteList(notes) {
        const notesListContainer = this.root.querySelector(".notes__list");

        notesListContainer.innerHTML = "";

        for (const note of notes) {
            const html = this._createListItemHTML(note.id, note.title, note.body, new Date(note.updated));

            notesListContainer.insertAdjacentHTML("beforeend", html);
        }

        notesListContainer.querySelectorAll(".notes__list-item").forEach(noteListItem => {
            noteListItem.addEventListener("click", () => {
                this.onNoteSelect(noteListItem.dataset.noteId);
            });
            noteListItem.addEventListener("dblclick", () => {
                function showModal() {
                    const modal = document.getElementById("myModal");
                    const desfoque = document.getElementById("desfoque");
                    modal.style.visibility = "visible";
                    desfoque.style.visibility = "visible"
                }
                showModal()

                function hideModal() {
                    const modal = document.getElementById("myModal");
                    const desfoque = document.getElementById("desfoque");
                    modal.style.visibility = "hidden";
                    desfoque.style.visibility = "hidden"
                }

                const yes = document.getElementById("yes")
                const no = document.getElementById("no")

                yes.addEventListener('click', () => {
                    this.onNoteDelete(noteListItem.dataset.noteId); 
                    hideModal() 
                })
                no.addEventListener('click', () => {
                    hideModal()  
                })
            });
        });
    }

    updateActiveNote(note) {
        this.root.querySelector(".notes__title").value = note.title;
        this.root.querySelector(".notes__body").value = note.body;

        this.root.querySelectorAll(".notes__list-item").forEach(noteListItem => {
            noteListItem.classList.remove("notes__list-item--selected");
        });

        this.root.querySelector(`.notes__list-item[data-note-id="${note.id}"]`).classList.add("notes__list-item--selected");
    }

    updateNotePreviewVisibility(visible) {
        this.root.querySelector(".notes__preview").style.visibility = visible ? "visible" : "hidden";
    }


}